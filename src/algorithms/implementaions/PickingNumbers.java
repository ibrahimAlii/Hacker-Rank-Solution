...
/*
  Given an array of integers, find and print the maximum number of integers you can select from the array such that
   the absolute difference between any two of the chosen integers is less than or equal to .
    For example, if your array is , you can create two subarrays meeting the criterion:  and . The maximum length subarray has  elements.
...
public class PickingNumbers {
    ...
    public static int pickingNumbers(List<Integer> a) {
        ...
        int maxLength = 0, newMaxLength = 0;

        for (int i = 0; i < a.size(); i++) {
            for (Integer integer : a) {
                if (a.get(i) == integer || integer == a.get(i) + 1)
                    newMaxLength++;
            }
